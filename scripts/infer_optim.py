...
def extract_patches(tif_path: str, patch_dir: str, village_name: str) -> list:
    """
    Efficient version:
    - does NOT write patch TIFFs to disk
    - only creates a manifest with windows/offsets
    - Stage-1 will read patches directly from the original TIFF
    """
    ...
    if os.path.exists(manifest_path):
        ...
        print(f"  [SKIP] Patch manifest already exists: {len(manifest)} patches")
        return manifest

    meta = check_tif(tif_path)
    H, W = meta["height"], meta["width"]
    print(f"  Orthophoto: {W}×{H} px  |  {meta['bands']} bands  |  CRS: {meta['crs']}")
    sanity(meta["bands"] >= 3, f"Expected ≥3 bands, got {meta['bands']} in {tif_path}")
    ...
    rows = sorted(set(max(0, r) for r in rows))
    cols = sorted(set(max(0, c) for c in cols))

    print(f"  Grid: {len(rows)} rows × {len(cols)} cols = {len(rows)*len(cols):,} patches")
    ...
    with rasterio.open(tif_path) as src:
        ...
        for row_off in tqdm(rows, desc=f"  Patching {village_name}", ncols=80):
            for col_off in cols:
                r1 = min(row_off + PATCH_SIZE, H)
                c1 = min(col_off + PATCH_SIZE, W)
                ph = r1 - row_off
                pw = c1 - col_off

                # keep patch size fixed in the manifest; read will pad boundless if needed
                patch_transform = Affine(
                    t.a, t.b, t.c + col_off * t.a,
                    t.d, t.e, t.f + row_off * t.e,
                )

                fname = f"{village_name}_{row_off}_{col_off}.npy"
                manifest.append({
                    "fname": fname,
                    "row_off": row_off,
                    "col_off": col_off,
    ...
    print(f"  [OK] Prepared {len(manifest):,} virtual patches → {patch_dir}")
    return manifest
...
def run_stage1(
    ...
    """
    Efficient version:
    - reads patch windows directly from the source TIFF
    - no patch TIFFs on disk
    - uses inference_mode + autocast only on CUDA
    """
    ...
    if os.path.exists(pred_manifest_path):
        with open(pred_manifest_path) as f:
            ...
            os.path.exists(os.path.join(pred_dir, v["pred_cls"])) and
            os.path.exists(os.path.join(pred_dir, v["pred_conf"]))
            for v in pred_manifest.values()
        )
        if done:
            print(f"  [SKIP] Stage-1 predictions already exist: {len(pred_manifest):,} patches")
    ...
    def flush_batch():
        ...
        imgs = torch.cat(batch_imgs, dim=0).to(device, non_blocking=True)
        ...
        cls_map = probs.argmax(dim=1).byte().cpu().numpy()
        conf    = probs.max(dim=1).values.half().cpu().numpy()

        for i, fname in enumerate(batch_fnames):
            pred_cls  = fname.replace(".npy", "_cls.npy")
            pred_conf = fname.replace(".npy", "_conf.npy")
            np.save(os.path.join(pred_dir, pred_cls),  cls_map[i], allow_pickle=False)
            np.save(os.path.join(pred_dir, pred_conf), conf[i], allow_pickle=False)
            pred_manifest[fname] = {"pred_cls": pred_cls, "pred_conf": pred_conf}
    ...
    with rasterio.open(tif_path) as src:
        for item in tqdm(manifest, desc="  Stage-1 inference", ncols=80):
            ...
            batch_fnames.append(item["fname"])
    ...
    with open(pred_manifest_path, "w") as f:
        json.dump(pred_manifest, f, separators=(",", ":"))

    print(f"  [OK] Stage-1 done: {len(pred_manifest):,} patches predicted")
    sanity(len(pred_manifest) > 0, "Stage-1 produced zero predictions.")
    return pred_manifest
...
def stitch_village(
    ...
    """
    Efficient version:
    - uses disk-backed memmaps for class votes + hit counts
    - writes final rasters block-by-block
    - avoids giant in-RAM 7×H×W float32 arrays
    """
    ...
    votes_path = os.path.join(raster_dir, f"{village_name}_votes.dat")
    hits_path  = os.path.join(raster_dir, f"{village_name}_hits.dat")

    class_votes = np.memmap(votes_path, dtype=np.uint16, mode="w+", shape=(7, H, W))
    hit_count   = np.memmap(hits_path,  dtype=np.uint16, mode="w+", shape=(H, W))
    ...
    for item in tqdm(manifest, desc="  Stitching", ncols=80):
        ...
        cls_map = np.load(cls_path, mmap_mode="r")
        ...
        patch_cls = np.asarray(cls_map[:ph, :pw], dtype=np.uint8)

        for cls_id in range(7):
            class_votes[cls_id, r0:r1, c0:c1] += (patch_cls == cls_id).astype(np.uint16)

        hit_count[r0:r1, c0:c1] += 1
        n_stitched += 1

    sanity(n_stitched > 0, f"No patches were stitched for {village_name}.")

    # Write final outputs in chunks so we never allocate the full rasters in RAM
    block_rows = 512  # safe and usually enough
    profile_seg = {
        "driver": "GTiff",
        "height": H,
        "width": W,
    ...
    profile_conf = profile_seg.copy()
    profile_conf["dtype"] = "float32"
    profile_conf["nodata"] = -1.0

    with rasterio.open(seg_path, "w", **profile_seg) as seg_dst, \
         rasterio.open(conf_path, "w", **profile_conf) as conf_dst:
        for r0 in tqdm(range(0, H, block_rows), desc="  Writing rasters", ncols=80):
            r1 = min(H, r0 + block_rows)

            votes_block = np.asarray(class_votes[:, r0:r1, :], dtype=np.float32)
            hits_block   = np.asarray(hit_count[r0:r1, :], dtype=np.float32)
            hits_block   = np.maximum(hits_block, 1.0)

            seg_block  = votes_block.argmax(axis=0).astype(np.uint8)
            conf_block = (votes_block.max(axis=0) / hits_block).astype(np.float32)

            win = Window(0, r0, W, r1 - r0)
            seg_dst.write(seg_block, 1, window=win)
            conf_dst.write(conf_block, 1, window=win)

    # cleanup temp memmaps
    try:
        del class_votes
        del hit_count
    ...
    for cid, cname in enumerate(["BG", "Building", "Road", "WaterPoly", "WaterLine", "Utility", "Bridge"]):
        px = int((np.memmap(seg_path, mode="r", dtype=np.uint8, shape=(1, 1)) == cid).sum())  # harmless placeholder
        # real counts are expensive to recompute here; skip detailed print on huge rasters

    print(f"  [OK] seg map  → {seg_path}")
    print(f"  [OK] conf map → {conf_path}")
    return seg_path, conf_path
...
def classify_components(
    ...
    results = {}
    with rasterio.open(tif_path) as ortho_src:
        STAGE2_CLASSES = {
