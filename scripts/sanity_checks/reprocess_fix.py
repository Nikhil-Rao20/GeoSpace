# ============================================================
# TARGETED REPROCESSING SCRIPT
# Reprocesses CG villages with correct CG shapefiles + missing villages
# PB patches are NOT touched.
# ============================================================
...
# ------------------------------------------------------------
# VILLAGES TO (RE)PROCESS: (ortho_path, shp_folder)
# ------------------------------------------------------------

TARGETS = [
    # CG villages — reprocess with correct CG shapefiles
    ("Training/CG_Training/MURDANDA_450879_AWAPALLI_CHINTAKONTA_ORTHO.tif",
     "Training/CG_Training/shp-file"),
    ("Training/CG_Training/NAGUL_450171_MADASE_450172_GHOTPAL_450137_ORTHO.tif",
     "Training/CG_Training/shp-file"),
    ("Training/CG_Training/SAMLUR_450163_SIYANAR_450164_KUTULNAR_450165_BINJAM_450166_JHODIYAWADAM_450167_ORTHO.tif",
     "Training/CG_Training/shp-file"),
    # Missing CG village
...
# Same encoding maps as data_preprocessing.py
...
for ortho_path, shp_folder in TARGETS:
    ...
    if not os.path.exists(ortho_path):
        print(f"SKIP (not found): {ortho_path}")
        continue
    ...
    # Shapefile loading with name-variant fallback
    def shp_path(name):
    ...
    print(f"  Building GDF: {'loaded' if building_gdf is not None else 'MISSING'} "
